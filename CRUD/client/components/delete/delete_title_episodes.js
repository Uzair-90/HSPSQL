...
function DeleteTitleEpisodes() {
  const [tconst, setTconst] = useState('');
  const [message, setMessage] = useState('');

  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://localhost:3001/api/title_episodes/${tconst}`);
  ...
  return (
    <form>
      ...
      <button className='glow-on-hover'  onClick={handleDelete}>Delete Title Episodes</button>
      <div>{message}</div>
    </form>
  );
}

export default DeleteTitleEpisodes;
