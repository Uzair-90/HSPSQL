...
function DeleteTitlePrincipals() {
  const [tconst, setTconst] = useState('');
  const [message, setMessage] = useState('');

  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://localhost:3001/api/title_principals/${tconst}`);
  ...
  return (
    <form>
      ...
      <button className='glow-on-hover' onClick={handleDelete}>Delete Title Principals</button>
      <div>{message}</div>
    </form>
  );
}

export default DeleteTitlePrincipals;
